...
-- Q3. what are top 3 values of in total invoice?
select * from invoice
order by total desc
limit 3
...
Select artist.name as artists ,count(*) as c from artist 
...
-- Method 3: Without Window function
SELECT 
    customer.country, 
...
HAVING COUNT(*) = (
    ...
    FROM (
        ...
        JOIN customer cust ON cust.customer_id = inv.customer_id
        JOIN track tr ON tr.track_id = il.track_id
        JOIN genre g ON g.genre_id = tr.genre_id
        WHERE cust.country = customer.country
        GROUP BY g.genre_id
    ) sub
);
